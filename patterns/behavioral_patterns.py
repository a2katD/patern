...
class SmsNotifier(Observer):
    def update(self, subject, user):
        print(f'SMS-сообщение: к нам присоединился '
              f'{subject.students[-1].name if user == "student" else subject.teachers[-1].name}')


class EmailNotifier(Observer):
    def update(self, subject, user):
        print(f'EMAIL-сообщение: к нам присоединился '
              f'{subject.students[-1].name if user == "student" else subject.teachers[-1].name}')
